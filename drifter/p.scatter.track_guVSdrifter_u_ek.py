...
def clean_outliers(gu, ve, ek, max_speed=3.0, mad_thresh=3.5):
    """
    ...
    mask = np.isfinite(gu) & np.isfinite(ve) & np.isfinite(ek)
    # Step 2: physical limit
    mask &= (np.abs(gu) <= max_speed) & (np.abs(ve) <= max_speed) & (np.abs(ek) <= max_speed)
    ...
    mask &= ~mad_based_outlier(ek, mad_thresh)

    return gu[mask], ve[mask], ek[mask], mask

sat_here = sys.argv[1]
df1 = pd.read_csv(f"track_guvVSnearby_drifters_at_intersection_points_{sat_here}_w_ekman.csv")

gu_clean, ve_clean, ek_clean, mask = clean_outliers(df1["gu"], df1["ve"], df1["ek"])

df = pd.DataFrame({
    "gu_mask": gu_clean,
    "ve_mask": ve_clean,
    "ek_mask": ek_clean,
    "gu_ek": gu_clean + ek_clean
})
# Create a scatter plot
plt.figure(figsize=(8, 6))
plt.scatter(df["gu_ek"], df["ve_mask"], alpha=0.5)
plt.title("Scatter Plot of gu+ek vs ve")
plt.xlabel("track gu+ek")
plt.ylabel("drifter ve")
plt.grid(True)

# Calculate the correlation coefficient
correlation = df["gu_ek"].corr(df["ve_mask"])
...
plt.legend()
# Save the plot
plt.savefig(f'scatter_plot_{sat_here}_track_uVSnearby_drifter_u_ek.png')
plt.show()
