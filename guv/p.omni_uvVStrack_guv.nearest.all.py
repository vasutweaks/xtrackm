...
for omni_id in omni_d.keys():
    ...
    N = omni_v.count().item()
    print(N)
    df_4 = find_n_closest_points(df_all, lon_omni, lat_omni, n=1)
    # print(omni_id, lon_omni, lat_omni, "\n", df_4.head(1))
    sat = df_4["sat"].values[0]
    track_number_self = df_4["track_self"].apply(lambda x: str(x)).values[0]
    track_number_other = df_4["track_other"].apply(lambda x: str(x)).values[0]
    lons_inter1 = df_4["lons_inter"].values[0]
    lats_inter1 = df_4["lats_inter"].values[0]
    lon1 = lons_inter1
    lat1 = lats_inter1
    x_from_coast_self1 = df_4["x_from_coast_self"].values[0]
    x_from_coast_other1 = df_4["x_from_coast_other"].values[0]
    angle_acute = df_4["angle_acute"].values[0]
    angle_obtuse = df_4["angle_obtuse"].values[0]
    print(
        sat,
        omni_id,
        track_number_self,
